// gcc -Wall -pedantic UDP-client.c -l ws2_32 -o client.exe
	...
	#include <stdint.h> // for int32_t (line 129)
	int OSInit( void )
	{
		WSADATA wsaData;
...
int initialization( struct sockaddr ** internet_address, socklen_t * internet_address_length )
{
	...
	int getaddrinfo_return = getaddrinfo( "192.168.2.99", "24042", &internet_address_setup, &internet_address_result );
...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	for (int i = 0; i < packets; i++)
	{
		...
		{
		//aantal pakketen tonen hoeveel de gebruiker heeft ingegeven
			buffer[number_of_bytes_received] = '\0';
			printf( "Received : %s\n", buffer );
			timesReceived++;
		}
	}
	printf(" -->  %d succeeded from the %d times. <--", timesReceived, packets);
